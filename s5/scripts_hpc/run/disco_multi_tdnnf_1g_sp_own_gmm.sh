...
discophone_root=/tudelft.net/staff-bulk/ewi/insy/SpeechLab/siyuanfeng/software/kaldi/egs/discophone/v1${lang_name_suffix}_multilang_phones_lms #v1_phones_lms
...
lang_name=universal
ivector_extractor=$discophone_root/exp/nnet3${nnet3_affix}/${lang_name}/extractor

if $use_ivector; then
  dir=exp/chain_discophone_lat_label/${lang_name}${lang_name_suffix}_${LMTYPE}/lat_gen_acwt${lat_generator_acwt}/tdnn${tdnn_affix}_sp_${train_set}_own_gmm #_epochs$num_epochs
else
  dir=exp/chain_discophone_lat_label/${lang_name}${lang_name_suffix}_${LMTYPE}/lat_gen_acwt${lat_generator_acwt}/tdnn${tdnn_affix}_noivec_sp_${train_set}_own_gmm
fi
train_ivector_dir=exp/chain_discophone_lat_label/nnet3/ivectors_${lang_name}${lang_name_suffix}_mboshi_hires/${train_set}_sp
...
ali_dir=${gmm_dir}_ali_sp #exp/chain_discophone_lat_label/${lang_name}_${LMTYPE}/lat_gen_acwt${lat_generator_acwt}/${train_set}_ali
tree_dir=exp/chain_discophone_lat_label/${lang_name}${lang_name_suffix}_${LMTYPE}/lat_gen_acwt${lat_generator_acwt}/tree_sp_bi${tree_affix}_own_gmm
lat_dir=exp/chain_discophone_lat_label/${lang_name}${lang_name_suffix}_${LMTYPE}/lat_gen_acwt${lat_generator_acwt}/${train_set}_sp_lats_own_gmm
...
train_data_dir_sp=data_hires_pitch_conf_discophone/${train_set}_sp
...
lores_train_data_dir_sp=data_plus_discophone_transcripts_collapse_ali/${lang_name}${lang_name_suffix}_${LMTYPE}/acwt${lat_generator_acwt}/${train_set}_sp
...
if [ $stage -le 5 ] && [ $stop_stage -gt 5 ]; then
  echo "creating speed perturbed data to $lores_train_data_dir_sp"
  utils/data/perturb_data_dir_speed_3way.sh $lores_train_data_dir $lores_train_data_dir_sp
  echo "making MFCC features for low-resolution speed-perturbed data"
  steps/make_mfcc.sh --cmd "$train_cmd" --nj $nj $lores_train_data_dir_sp || exit 1
  steps/compute_cmvn_stats.sh $lores_train_data_dir_sp || exit 1
  utils/fix_data_dir.sh $lores_train_data_dir_sp || exit 1 
fi

if [ $stage -le 6 ] && [ $stop_stage -gt 6 ]; then
  echo "aligning with the perturbed low-resolution data"
  steps/align_fmllr.sh --nj $nj $lores_train_data_dir_sp $discophone_root/data/lang_combined_test $gmm_dir $ali_dir
fi

if [ $stage -le 7 ] && [ $stop_stage -gt 7 ]; then
  if [ ! -d $train_data_dir_sp ]; then
    echo "creating high-resolution MFCC features of speed perturbed data"
    utils/copy_data_dir.sh $lores_train_data_dir_sp $train_data_dir_sp
    mkdir -p $train_data_dir_sp/.backup/
    mv $train_data_dir_sp/text $train_data_dir_sp/.backup/
    steps/make_mfcc_pitch.sh --mfcc-config conf/mfcc_hires_from_${hires_conf_set}.conf --nj $nj $train_data_dir_sp || exit 1
    steps/compute_cmvn_stats.sh $train_data_dir_sp || exit 1
  else
    echo "$train_data_dir_sp already existed. Skip creating MFCC_hires"
  fi 
fi

if [ $stage -le 8  ] && [ $stop_stage -gt 8 ] ; then
  if [ -f $train_ivector_dir/ivector_online.scp ]; then
    echo "$0: Skipping ivector extraction as $train_ivector_dir already exists"
  else
    echo "$0: extracting ivectors"
    input_data=$data_dir_name_nopitch/${train_set}_sp
    if [ ! -d $input_data ]; then
      utils/data/limit_feature_dim.sh 0:39 $train_data_dir_sp $input_data || exit 1
      steps/compute_cmvn_stats.sh $input_data || exit 1
    fi
    output_data=$train_ivector_dir #exp/chain_${hires_conf_set}_lat_label/nnet3/ivectors_${lang_name}${lang_name_suffix}_mboshi_hires/${train_set}_sp
...
if [ $stage -le 15 ] && [  $stop_stage -gt 15 ]  ; then
  ...
  steps/align_fmllr_lats.sh --stage $align_fmllr_lats_stage --nj $nj --cmd "$train_cmd" --beam $align_fmllr_lats_beam --retry-beam $align_fmllr_lats_retry_beam ${lores_train_data_dir_sp}\
...
if [ $stage -le 16 ] && [ $stop_stage -gt 16 ] ; then
  ...
  steps/nnet3/chain/build_tree.sh --frame-subsampling-factor 3 \
      ...
      --cmd "$train_cmd" 4000 ${lores_train_data_dir_sp} $discophone_root/data/lang_chain/lang_$lang_name $ali_dir $tree_dir
fi
xent_regularize=0.1
if [ $stage -le 17 ] && [ $stop_stage -gt 17 ]  ; then
  ...
  feat_dim=$(feat-to-dim scp:${train_data_dir_sp}/feats.scp -)
...
if [ $stage -le 19 ]  && [ $stop_stage -gt 19 ] ; then
  ...
  # we do not use $train_data_dir_sp as in AUD task _sp not needed 
  ...
  --cmd run.pl --nj 2 --ivector-dir $ivector_dir $bnf_name $input_data $output_data $dir  || exit 1
  if ! $bin_output ; then
