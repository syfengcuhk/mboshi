...
libri_light_set=unlab_600_full
. cmd.sh
. ./path.sh
...
lang_name=universal
input_feat_affix=apc_libri_light_${libri_light_set}
if $use_ivector; then
  dir=exp/chain_discophone_lat_label/${lang_name}${lang_name_suffix}_${LMTYPE}/lat_gen_acwt${lat_generator_acwt}/other_input_feats/tdnn${tdnn_affix}${input_feat_affix}${data_aug_suffix}_${train_set}_own_gmm #_epochs$num_epochs
else
  dir=exp/chain_discophone_lat_label/${lang_name}${lang_name_suffix}_${LMTYPE}/lat_gen_acwt${lat_generator_acwt}/other_input_feats/tdnn${tdnn_affix}_noivec${input_feat_affix}${data_aug_suffix}_${train_set}_own_gmm
fi
...
train_data_dir=/tudelft.net/staff-bulk/ewi/insy/SpeechLab/siyuanfeng/Autoregressive-Predictive-Coding/exp/${libri_light_set}/apc_5L100_feat13_mfcc_cm_e100_tshift5_res.dir/extracted_mboshi_feats_no_pad/${train_set}_-1/data_for_kaldi #data_hires_pitch_conf_discophone/${train_set}
...
common_egs_dir=exp/chain_discophone_lat_label/${lang_name}${lang_name_suffix}_${LMTYPE}/lat_gen_acwt${lat_generator_acwt}/other_input_feats/tdnn1g_lr2.5e-4apc_libri_light_${libri_light_set}_full_own_gmm/egs #exp/chain_discophone_lat_label/universal_phn_ug_multi/lat_gen_acwt10.0/tdnn1g_full_own_gmm
for f in  $train_data_dir/feats.scp $lores_train_data_dir/feats.scp ; do
  [ ! -f $f ] && echo "$0: expected file $f to exist" && exit 1
done

if [ ! -z $common_egs_dir ] &&  [ ! -d $common_egs_dir ]; then
  echo "common_egs_dir appointed, but dir not found, check again"
  exit 0
fi
...
if [ "$clustering_algo"   == "kmeans"  ]; then
  ...
  clustering_suffix="_seg_hdbscan_clusters${nclusters}_rand_${rand_state}"
else
    echo "non supported  clustering algo"
fi
