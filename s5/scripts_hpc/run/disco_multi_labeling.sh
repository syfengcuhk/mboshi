...
if [ $stage -le 11  ] && [ $stop_stage -gt 11 ]; then
  ...
  for set in full; do
    ...
    gmm_graph_dir=$gmm_model/graph
    $cmd JOB=1:$num_jobs $input_dir/log/lattice_best_path.JOB.log \
      lattice-best-path --lm-scale=0.001 "ark:gunzip -c $input_dir/lat.JOB.gz|" "ark,t:|utils/int2sym.pl -f 2- $gmm_graph_dir/words.txt > $input_dir/text.JOB" "ark:|gzip -c - > $input_dir/ali.JOB.gz" || exit 1;
    cat $input_dir/text.* > $input_dir/text || exit 1; 
  done
fi
...
if [ $stage -le 12 ] && [ $stop_stage -gt 12 ];then
  echo "$0: get phone sequences from model-level alignments, alignments got from lattice-best-path"
  for set in  full; do
    dir=$gmm_model/decoding_for_mboshi_${set}_acwt${acwt} #$model/decoding_for_mboshi_${LMTYPE}_${set}_acwt${acwt}
    num_jobs=$(cat $dir/num_jobs) || exit 1
    gmm_graph_dir=$gmm_model/graph
    output_dir=exp/chain_${hires_conf_set}_lat_label/${lang_name}${lang_name_suffix}_${LMTYPE}/lat_gen_acwt${acwt}/${set}_phone_ali_from_gmm_lattice
    mkdir -p $output_dir
    if $ctm_format; then
      $cmd JOB=1:$num_jobs $output_dir/log/get_ctm.JOB.log \
        ali-to-phones --frame-shift=0.03 --ctm-output $gmm_model/final.mdl "ark:gunzip -c $dir/ali.JOB.gz|" "|utils/int2sym.pl -f 5- $gmm_graph_dir/phones.txt > $output_dir/ctm.JOB.txt " || exit 1;
      cat $output_dir/ctm.*.txt > $output_dir/ctm.txt || exit 1
      rm -f $output_dir/ctm.*.txt
    else
      $cmd JOB=1:$num_jobs $output_dir/log/get_phone_ali.JOB.log \
        ali-to-phones --per-frame $gmm_model/final.mdl "ark:gunzip -c $dir/ali.JOB.gz|" ark,t:- \| utils/int2sym.pl -f 2- $gmm_graph_dir/phones.txt \> $output_dir/phone_ali.JOB.ali
      cat $output_dir/phone_ali.*.ali > $output_dir/phone_ali.ali || exit 1
      rm -f $output_dir/phone_ali.*.ali
    fi
    cp $gmm_graph_dir/phones.txt $output_dir/phones.txt
  done
...
if [ $stage -le  13 ] && [ $stop_stage -gt 13 ]; then
  ...
  eval_set=full
#  if $eval_from_lattice; then
  output_dir=exp/chain_${hires_conf_set}_lat_label/${lang_name}${lang_name_suffix}_${LMTYPE}/lat_gen_acwt${acwt}/${eval_set}_phone_ali_from_gmm_lattice
    hyp_trans=$output_dir/phone_ali.ali
#  else
#    output_dir=exp/chain_${hires_conf_set}_lat_label/${lang_name}${lang_name_suffix}_${LMTYPE}/lat_gen_acwt${acwt}/${eval_set}_phone_ali
    hyp_trans=$output_dir/phone_ali.ali
